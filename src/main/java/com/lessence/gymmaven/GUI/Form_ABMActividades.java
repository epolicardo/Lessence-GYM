    ...
    private void jBAñadirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBAñadirActionPerformed
        ...
        if (jTActividades.getText().equals("")) {
            JOptionPane.showMessageDialog(this,
                    "Debe ingresar el nombre de una actividad",
                    "ABM Actividades", JOptionPane.ERROR_MESSAGE);

        } else {
            Actividades actividad = new Actividades();
            actividad.setActividad(jTActividades.getText());
            sesion.beginTransaction();
            sesion.save(actividad);
            sesion.getTransaction().commit();
            sesion.close();
            JOptionPane.showMessageDialog(this,
                    "La actividad fue cargada con éxito",
                    "ABM Actividades", JOptionPane.INFORMATION_MESSAGE);
    ...
    private void jTaActividadesMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTaActividadesMouseClicked
        if (evt.getClickCount() == 2) {
            ...
            for (int i = 0; i < lista_actividades.size(); i++) {
                ...
                if (modelo.getValueAt(jTaActividades.getSelectedRow(), 1).toString().equals(actividad.getActividad())) {
